...
class user_basic(models.Model):
    ...
    phone = models.CharField(max_length=20,null=True,blank=True,verbose_name='固定电话')
    ...
    can_share_out = models.BooleanField(null=False,blank=True,default=False,verbose_name='是否能享受商品分红')
    class Meta:
...
class user_adder(models.Model):
    added = models.ForeignKey(user_basic,related_name="user_add_from",null=False,verbose_name='被添加者（会员）')
    adding = models.ForeignKey(user_basic,related_name="user_add_to",null=False,verbose_name='添加者（报单中心）')
    class Meta:
